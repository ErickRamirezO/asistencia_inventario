...
const ReporteAsistencia = () => {
  ...
  const usuariosDisponibles = todosUsuarios.filter(
    (usuario) =>
      !usuariosSeleccionados.some(
        (seleccionado) => seleccionado.id === usuario.id
      )
  );
  ...
  const eliminarUsuario = (id) => {
    setUsuariosSeleccionados(
      usuariosSeleccionados.filter((usuario) => usuario.id !== id)
    );
  };
  ...
  const verificarDatosReporte = async () => {
    ...
    try {
      ...
      const response = await api.post("/reportes/asistencia/has-data", {
        usuarioIds,
        fechaInicio,
        fechaFin,
      });
  ...
  const descargarReporte = async (reportFormat) => {
    // Añadimos reportFormat como parámetro
    ...
    if (!hayDatos) {
      toast.warning(
        "No hay datos para el reporte en ese rango y usuarios seleccionados",
        {
          richColors: true,
        }
      );
    ...
    try {
      ...
      if (error.response && error.response.data) {
        ...
        toast.error("Error al generar reporte", {
          description:
            "No se pudo conectar con el servidor o hubo un problema de red.",
          richColors: true,
          id: toastId,
        });
        crearLog(
  ...
  const asistenciaPorUsuario = asistenciaPreview.reduce((acc, registro) => {
    ...
    const toMinutes = (time) => {
      if (
        typeof time !== "string" ||
        time.includes("Aún no completa asistencia para cálculo")
      ) {
        return 0;
      }
    ...
    >
        ...
        >
            ...
            <div className="space-y-3">
              ...
              <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger asChild>
                  ...
                  >
                    <span className="text-xs md:text-[13px] sm:text-sm">
                      Seleccionar usuarios...
                    </span>
              ...
              <div className="text-xs md:text-[13px] sm:text-sm text-muted-foreground mt-2">
                {usuariosDisponibles.length} usuarios disponibles para
                seleccionar
              </div>
            </div>
                ...
                {usuariosSeleccionados.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-center text-sm text-muted-foreground">
                    ...
                    <p className="text-xs mt-1">
                      Seleccione al menos un usuario para el reporte
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="border rounded-md p-2 sm:p-3 min-h-[120px] sm:min-h-[200px] bg-muted/30">
                ...
                <div className="mt-4">
                  {dateRange?.from && dateRange?.to ? (
                    ...
                    <p className="text-xs md:text-[13px] sm:text-sm text-muted-foreground mt-4">
          ...
          <div className="mt-8 sm:mt-8">
            <h3 className="font-semibold mb-2 text-sm sm:text-sm">
              Vista previa de asistencia
            </h3>
            {cargandoPreview ? (
              <div className="text-xs md:text-[13px] sm:text-sm">
                Cargando asistencia...
              </div>
            ) : asistenciaPreview.length === 0 ? (
              <div className="text-muted-foreground text-xs md:text-[13px] sm:text-sm">
                No hay registros para mostrar.
              </div>
            ) : (
              Object.entries(asistenciaPorUsuario).map(([key, data]) => (
                <div key={key} className="mb-8 sm:mb-8">
                  <div className="font-bold mb-2 text-xs md:text-[13px] sm:text-sm">
                    {data.registros[0].nombreCompletoUsuario} -{" "}
                    ...
                    <Table className="min-w-[700px] text-xs md:text-[13px] sm:text-sm">
                      ...
                      <TableBody>
                          ...
                          <TableRow key={idx}>
                            ...
                            <TableCell className="px-2 py-1 truncate">
                              {a.eventoNombre !== "Asistencia Normal" ? (
                                ...
                                <Badge
                                  className={getAlmuerzoBadgeColor(
                                    a.horaInicioAlmuerzoFormatted
                                      ? a.horaInicioAlmuerzoFormatted
                                      : "Pendiente"
                                  )}
                                >
                        ...
                        <TableRow>
                          <TableCell
                            colSpan={8}
                            className="px-2 py-1 font-bold text-right"
                          >
                            Total:
                          </TableCell>
        ...
        <CardFooter className="flex justify-center border-t pt-4 sm:pt-6 gap-4">
          <Button
            ...
            size="lg"
            className="w-full max-w-xs text-xs md:text-[13px] sm:text-sm"
          >
